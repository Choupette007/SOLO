...
try:
    from solana_trading_bot_bundle.trading_bot.utils import get_cached_creation_time, cache_creation_time  # type: ignore
except Exception:
...
_ENRICH_TOKEN_TIMEOUT_S = float(os.getenv("PRICE_ENRICH_TOKEN_TIMEOUT_S", str(pe_cfg.get("token_timeout", 6))))
_SMALL_BACKOFF_S = float(os.getenv("PRICE_ENRICH_SMALL_BACKOFF_S", str(pe_cfg.get("small_backoff", 0.2))))
_HTTP_TOTAL_TIMEOUT_S = float(os.getenv("PRICE_ENRICH_HTTP_TIMEOUT_S", str(pe_cfg.get("http_timeout", 8))))

_PRICE_CACHE_TTL_POS_S = float(os.getenv("PRICE_CACHE_TTL_S", str(pe_cfg.get("price_cache_ttl", 60))))
_PRICE_CACHE_TTL_NEG_S = float(os.getenv("PRICE_CACHE_NEG_TTL_S", str(pe_cfg.get("price_cache_neg_ttl", 300))))
_PRICE_CACHE_MAX_SIZE  = int(os.getenv("PRICE_CACHE_MAX_SIZE", str(pe_cfg.get("price_cache_max_size", 5000))))
...
if not _fetch_delegate_ok:
    ...
    async def fetch_birdeye_multi_price(session: aiohttp.ClientSession, addrs: List[str]) -> Dict[str, Optional[Dict[str, Any]]]:
        ...
        for i in range(0, len(addresses), max_per_call):
            ...
            # After fallback attempt, handle 429 if it occurred
            if status == 429:
                retry_after = None
                try:
                    retry_after = float(resp_headers.get("Retry-After") or resp_headers.get("retry-after"))
