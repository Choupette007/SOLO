...
# Removed: duplicate/incomplete _enrich_shortlist_with_signals definition and module-scope enrichment block.
# The complete implementation is defined later in the file at line ~3807.
...
async def _enrich_shortlist_with_signals(tokens: List[Dict[str, Any]], cfg: dict | None = None) -> List[Dict[str, Any]]:
    """
    Enrich tokens with technical signals (Bollinger Bands, candlestick patterns).
    ...
    Returns:
        Enriched token list (or original tokens if enrichment fails/disabled).
    """
    ...
    try:
        ...
        # Pattern classification enrichment (if enabled and available)
        if patterns_enabled and _classify_patterns is not None:
            ...
            for tok in (enriched or []):
                try:
                    ...
                    try:
                        ...
                        # if _ensure_ohlcv5 fails for this token, log and skip
            ...
            try:
                if candidate_for_batch:
                    ...
                    try:
                        ...
                        # On exception, set error batch_summary
                        logger.warning("Batch attach bbands/patterns failed", exc_info=True)
                        batch_summary = {"n_attempted": 0, "n_bbands": 0, "n_patterns": 0, "errors": 1}
            except Exception:
                # degrade gracefully; will fall back to per-token helpers below
                pass
            
            # Safe metrics emission
            try:
                if METRICS is not None and hasattr(METRICS, "record"):
                    METRICS.record({
                        "type": "signals_batch_summary",
                ...
                logger.debug("Failed to emit metrics for batch summary", exc_info=True)
