...
def _new_async_client(rpc_url: str) -> AsyncClientType:
    ...
    try:
        ...
        # Caller is expected to tolerate None-like behavior or not call this when missing.
        raise
...
def _record_metric_fill(
    ...
    **extra_metadata
    ...
    Uses, in order of preference:
      ...
      - cached SOLâ†’USD price passed in extra_metadata['sol_price_usd'] or price_cache
    ...
    try:
        ...
        # Prefer a supplied SOLâ†’USD price (caller should pass sol_price_usd). Fall back to price_cache.
        try:
        ...
        def _resolve_decimals(mint_addr: str | None, fallback: int = 6) -> int:
            try:
                if not mint_addr:
                    return fallback
                # price_cache may carry decimals info under keys like "decimals:<mint>"
                ...
                try:
                    ...
                    if isinstance(platform_fee, dict):
                        ...
                        try:
                            ...
                            fee_amt_val = 0.0
                        # If fee appears to be in lamports (very large), treat as lamports else token base units
                        if fee_amt_val and fee_amt_val > LAMPORTS_PER_SOL_LOCAL:
                            # probably token base units? guard: if huge, convert using token decimals is ambiguous.
                            # Best-effort: assume lamports
        ...
        try:
            if qty and qty > 0 and amount_sol and (sol_price_usd and sol_price_usd > 0):
                # price_usd = (amount_sol * sol_price_usd) / qty
                ...
                # last resort: attempt to compute using amount_sol and qty if sol_price_usd unknown
...
# This ensures any other metrics adapter using that helper gets a fill event too.
try:
    _metrics_on_fill(
        token_addr=token_addr,
        symbol=symbol,
    ...
    # keep trading logic safe if metrics adapter misbehaves
    try:
        logger.debug("Aux metrics_on_fill invocation failed", exc_info=True)
    except Exception:
        pass# Also call the generic _metrics_on_fill wrapper as a best-effort side-channel.
# This ensures any other metrics adapter using that helper gets a fill event too.
try:
    _metrics_on_fill(
        token_addr=token_addr,
...
def _merge_by_address(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # NEW: final safeguard â€“ recompute/normalize creation_timestamp on outputs
...
async def _build_live_shortlist(
    *,
    ...
    """
    Live discovery â†’ merge â†’ dedupe â†’ pre-cut â†’ enrich â†’ diagnostics â†’ coalesce â†’ shortlist â†’ persist.
    ...
    for q in queries:
        ...
        for pair in (chunk or []):
            ...
            seen.add(addr)
            rows.append(pair)
        # Soft cap early if weâ€™re already above the intended working set
    ...
    # 3) PERSIST raw discovery for GUI â€œDiscovered Tokensâ€
    try:
    ...
    # 12) Persist shortlist for GUI â€œShortlist/Eligibleâ€ view
    try:
...
async def select_top_five_per_category(eligible_tokens: List[Dict]) -> List[Dict]:
    ...
    if total_cats == 0:
        ...
        # Size via env (default 5) so you donâ€™t need to change the signature
        try:
...
async def real_on_chain_sell(
    ...
    try:
        ...
        if hours_held >= exit_cfg["max_hold_hours"]:
            logger.info(f"[{symbol}] Max-hold {exit_cfg['max_hold_hours']}h reached Ã¢â€ â€™ exit full.")
            amount = _sell_amount_lamports(token_balance, 1.0, default_decimals=decimals)
            quote, error = await _safe_get_jupiter_quote(
                input_mint=token_address,
                output_mint="So11111111111111111111111111111111111111112",
        ...
        if not ts.get("tp1_done", False) and profit_ratio <= exit_cfg["stop_loss_x"]:
            logger.info(f"[{symbol}] Stop-loss hit at {profit_ratio:.2f}x (Ã¢â€°Â¤ {exit_cfg['stop_loss_x']}x) Ã¢â€ â€™ sell all.")
            amount = _sell_amount_lamports(token_balance, 1.0, default_decimals=decimals)
            quote, error = await _safe_get_jupiter_quote(
                input_mint=token_address,
                output_mint="So11111111111111111111111111111111111111112",
        ...
        if not ts.get("tp1_done", False) and profit_ratio >= exit_cfg["tp1_x"]:
            ...
            logger.info(f"[{symbol}] TP1 {profit_ratio:.2f}x Ã¢â€ â€™ sell {pct*100:.1f}% and start {exit_cfg['trail_pct_after_tp1']*100:.0f}% trailing.")
            quote, error = await _safe_get_jupiter_quote(
                input_mint=token_address,
                output_mint="So11111111111111111111111111111111111111112",
                amount=amount,
        ...
        if ts.get("tp1_done", False) and (not ts.get("tp2_done", False)) and profit_ratio >= exit_cfg["tp2_x"]:
            ...
            amount = _sell_amount_lamports(token_balance, pct, default_decimals=decimals)
            logger.info(f"[{symbol}] TP2 {profit_ratio:.2f}x Ã¢â€ â€™ sell {pct*100:.1f}% of remaining; widen trail to {exit_cfg['trail_pct_moonbag']*100:.0f}%.")
            quote, error = await _safe_get_jupiter_quote(
                input_mint=token_address,
                output_mint="So11111111111111111111111111111111111111112",
                amount=amount,
        ...
        if ts.get("tp1_done", False):
            ...
            if current_price <= active_floor:
                logger.info(f"[{symbol}] Trailing stop @ {current_price:.8f} SOL (floor {active_floor:.8f}) Ã¢â€ â€™ exit full.")
                amount = _sell_amount_lamports(token_balance, 1.0, default_decimals=decimals)
                quote, error = await _safe_get_jupiter_quote(
                    input_mint=token_address,
                    output_mint="So11111111111111111111111111111111111111112",
...
async def main() -> None:
    ...
    try:  # ==== OUTER TRY ====
        ...
        try:
            ...
            short_bh = (blockhash[:12] + "â€¦") if isinstance(blockhash, str) else str(blockhash)
                    ...
                    try:
                        if not candidates:
                            ...
                            if not sol_price:
                                ...
                                # Normalize SOLâ†’USD price once for the cycle and store in cache for metric helpers
                                try:
                                ...
                                for tok in candidates:
                                    ...
                                    if (config.get("bot") or {}).get("per_asset_sizing", True):
                                        try:
                                            # unwraps (sol, usd) â†’ sol inside the resolver
                                    ...
                                    try:
                                        ...
                                        pass
                                    logger.info(
                                        "BUY-CANDIDATE %s (%s): preflight OK, got full quote (lamports_in=%d, outâ‰ˆ%s).",
                                        symbol,
                                        token_addr,
                                        amount_lamports_full,
                                    ...
                                    if _dry_run or _send_disabled or _simulate:
                                        ...
                                        logger.info(
                                            "DRYRUN-BUY %s (%s): txid=%s amount_in_SOL=%.6f priceâ‰ˆ%s SOL/token",
                                            symbol,
                                            token_addr,
                                            txid,
                                            buy_amount_sol,
                                        ...
                                        # Record metrics for dry-run buy (include sol_price_usd)
                                        _record_metric_fill(
                                            token_addr=token_addr,
                                            symbol=symbol,
                                            side="BUY",
                                    ...
                                    try:
                                        ...
                                        logger.info(
                                            "LIVE-BUY %s (%s) executed: txid=%s priceâ‰ˆ%s SOL/token",
                                            symbol,
                                            token_addr,
                                            txid or "?",
                                            f"{buy_price_sol:.12f}"
                                        ...
                                        # Record metrics for live buy (include sol_price_usd)
                                        _record_metric_fill(
                                            token_addr=token_addr,
                                            symbol=symbol,
                                            side="BUY",
