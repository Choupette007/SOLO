...
# --- indicators / patterns aliases (so existing calls keep working) ---
try:
    # package-relative (when installed as a bundle)
    from .indicators import rsi as ind_rsi
    from .patterns import classify_patterns as classify_patterns_arrays
except Exception:
    # flat/absolute fallback (when running from a flat workspace)
    from indicators import rsi as ind_rsi  # type: ignore  # noqa: F401
    from patterns import classify_patterns as classify_patterns_arrays  # type: ignore  # noqa: F401
...
# --- Birdeye cooldown / host-block state (centralized) ------------------------
from typing import cast

_BIRDEYE_HOST_BLOCK_UNTIL: Dict[str, float] = {}  # host -> unix_ts
...
def _birdeye_block_host(host: str, seconds: float) -> None:
...
def _birdeye_cooldown(seconds: float) -> None:
                ...
                async with session.get(url, headers=hdrs, timeout=timeout) as resp:
                    ...
                    for pair in pairs:
                        ...
                        rows.append({
                            ...
                            "market_cap": _mcap,
                            "creation_timestamp": (created // 1000) if created else 0,  # seconds
        ...
        try:
            ...
            if not _should_stop() and is_enabled_raydium():
                try:
                    ...
                    # hard cap: if Raydium is slow, bail quickly and keep UI responsive
