...
async def is_token_eligible(
    ...
    max_price_change = _num(config.get("discovery", {}).get("max_price_change", 1000), 1000)
    if abs(price_change_24h) > max_price_change:
        reason = f"price_change_24h={price_change_24h}% too high (> {max_price_change}%)"
        reasons.append(reason)
        _log_reject(token_address, reason)
        token = dict(token); token["eligibility_reasons"] = reasons
        return False, categories
...
def score_token(token: Dict[str, Any], config: Dict[str, Any]) -> float:
    ...
    max_price_change_allowed = _num(config.get("discovery", {}).get("max_price_change", 1000), 1000)
    if price_change_24h > 0 and max_price_change_allowed > 0:
        score += min(price_change_24h / max_price_change_allowed, 1.0) * weights.get("price_change", 0.2)
...
def summarize_filter_outcomes(
    ...
    for t in all_candidates or []:
        ...
        try:
            ...
            pc = 0.0
        max_pc = float((cfg.get("discovery") or {}).get("max_price_change", 1000) or 1000)
        if abs(pc) > max_pc:
