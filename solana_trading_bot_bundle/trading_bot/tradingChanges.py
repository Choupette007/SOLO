...
import signal
import random
from typing import TYPE_CHECKING, Any, Dict, List, Set, Tuple, Optional, Callable, Awaitable, Type
...
def _new_async_client(rpc_url: str) -> AsyncClientType:
    """
    Return a new AsyncClient, doing a late import so runtime succeeds even when
    solana-py isn't available at import time.

    Raises ImportError if solana.rpc.async_api is not installed.
    """
    try:
        from solana.rpc.async_api import AsyncClient as _AC  # type: ignore[reportMissingImports]
        return _AC(rpc_url)
    except ImportError:
        # Caller is expected to tolerate ImportError or avoid calling this when not installed.
        raise
...
try:
    ...
    _persist_discovered_tokens = persist_discovered_tokens
except Exception:
    try:
        # Fallback: absolute package import (when running from repo root)
        ...
        # Log the full traceback so we can see exactly why import failed while still
        # providing safe stubs to keep the bot running in dry-run mode.
        logger.exception("Failed to import database helpers; installing safe stubs and continuing (exc follows):")

        async def get_token_trade_status(token_address: str) -> Optional[Dict[str, Any]]:
            return None

        async def get_cached_token_data(token_address: str) -> Optional[Dict[str, Any]]:
            return None

        async def get_cached_creation_time(token_address: str) -> Optional[int]:
            return None

        async def cache_token_data(token: Dict[str, Any]) -> None:
            return None

        async def cache_creation_time(token_address: str, creation_time: int) -> None:
            return None

        async def update_token_record(*args, **kwargs) -> None:
            return None

        async def db_add_to_blacklist(*args, **kwargs) -> None:
            return None

        async def db_load_blacklist() -> Set[str]:
            return set()

        async def mark_token_sold(*args, **kwargs) -> None:
            return None

        async def init_db(*args, **kwargs) -> None:
            return None

        async def clear_expired_blacklist(*args, **kwargs) -> None:
            return None

        async def review_blacklist(*args, **kwargs) -> None:
            return None

        # async no-op persist_eligible_shortlist so awaiters won't break
        async def persist_eligible_shortlist(*args, **kwargs) -> Optional[int]:
            return None

        # async optional placeholder
        async def persist_discovered_tokens(*args, **kwargs) -> Optional[int]:
            return None

        _persist_discovered_tokens = None

# Try to import the database module object for direct access; tolerate failure
_db = None
try:
    from . import database as _db
except Exception:
    try:
        import solana_trading_bot_bundle.trading_bot.database as _db  # type: ignore
    except Exception:
        _db = None
        logger.warning("Could not import database module; database-backed features disabled.")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                def _apply_tech_tiebreaker(tokens: list, config: dict, *, logger: Optional[Logger] = None) -> list:
...
async def _enrich_shortlist_with_signals(tokens: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    """
    # If batch enricher not available, do nothing
    ...
    if not signals_enabled:
        try:
            logger.debug("Signals disabled by config; returning enriched without attaching patterns.")
        except Exception:
            pass
        return enriched
...
def _derive_creation_ts_s(d: Dict[str, Any]) -> int:
    ...
    Handles msâ†’s, ISO8601 strings, ints/floats, and simple nested dicts.
    ...
    for key in (
        ...
        val = d.get(key)
        ts = _to_intish(val)
        if ts > 2_000_000_000:  # clearly milliseconds â†’ seconds
            ts //= 1000
        if ts > 0:
            return ts
    return 0
...
def _restore_or_compute_categories(tokens: list[dict]) -> list[dict]:
    ...
    If missing, compute a coarse cap-bucket and attach:
      - 'low_cap'    : mc < 100_000
      - 'mid_cap'    : 100_000 â‰¤ mc < 500_000
      - 'large_cap'  : mc â‰¥ 500_000
...
async def _resolve_buy_amount_sol(
    *,
    ...
    if isinstance(v, (tuple, list)) and v:
        # (amount_sol, usd_amount) â†’ amount_sol
        v = v[0]
...
try:
    ...
    # No-op fallback so the call site wonâ€™t crash if the symbol is missing
...
async def _enrich_tokens_with_price_change_lazy(session, tokens, logger, blacklist, failure_count):
    ...
    # 1) If enrichment is empty or not a list â†’ neutral fallback
    ...
    # 2) If all deltas are zero and price is â‰¤0 across the whole batch â†’ neutral fallback
    ...
    try:
        ...
        for item in result:
            ...
            # --- timestamp mapping; normalize msâ†’s and clamp future ---
            ts_candidates = [
                item.get("updated_at"),
            ...
            if ts_new is not None:
                ts_new = int(ts_new)
                if ts_new > 10_000_000_000:  # msâ†’s
                    ts_new //= 1000
                now = int(time.time())
...
def acquire_single_instance_or_explain(stale_after_s: int = 180) -> bool:
    ...
    # There is a previous PID fileâ€”check staleness
...
async def _enrich_shortlist_with_signals(tokens: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Resolve fetcher early (may be None)
    try:
        fetcher = _resolve_ohlcv_fetcher()
    except Exception:
        fetcher = None
    ...
    async def _ensure_ohlcv5(tok: dict):
        ...
        # Fallback to fetcher with timeout
        try:
            addr = tok.get("address") or tok.get("token_address") or tok.get("mint") or (tok.get("baseToken") or {}).get("address")
            if not addr or not callable(fetcher):
                return None
    ...
    # Build candidate list and attach signals
    try:
        candidate_for_batch: list[dict] = []
        _min_bars = int((_signals_cfg.get("patterns") or {}).get("min_bars", 20))
        _bb_window = int((_signals_cfg.get("bbands") or {}).get("window", 20))
        ...
        # Per-token fallback attach where needed
        for tok in (enriched or []):
            try:
                if any(k.startswith("pat_") for k in tok.keys()):
                    continue
    ...
    try:
        ...
        if not assoc_addr_exists:
            ...
            if dry_run:
                ...
                logger.info(
                    "Watch-only: no ATA yet for %s (%s) â€” expected in dry-run.",
                ...
                if not assoc_addr:
                    # Live mode and creation failed â†’ log + blacklist to avoid repeated retries
                    logger.error("ATA creation unavailable for %s (%s); blacklisting.", symbol, token_address)
                    if token_address not in WHITELISTED_TOKENS:
                        await db_add_to_blacklist(token_address, "Failed to create token account")
                        blacklist.add(token_address)
...
async def real_on_chain_sell(
    ...
    try:
        ...
        if not assoc_addr_exists:
            if dry_run:
                # In dry-run we expect no ATA until a real buy happens; don't blacklist
                logger.info("Watch-only: no ATA yet for %s (%s) â€” expected in dry-run.", symbol, token_address)
                return None
            else:
        ...
        if not status or status.get("buy_price") is None or status.get("buy_time") is None:
            if dry_run:
                ...
                logger.info("No valid buy record for %s (%s) â€” expected in dry-run.", symbol, token_address)
                return None
            else:
        ...
        if (not ts.get("tp1_done", False)
            ...
            logger.info(
                ...
                f"profit {profit_ratio:.2f}x < {exit_cfg['no_tp_min_profit_x']}x Ã¢â€ â€™ exit full."
            )
...
async def main() -> None:
    ...
    try:  # ==== OUTER TRY ====
        ...
        # DB init
        try:
            # Prefer calling via module alias to avoid NameError if the local symbol is missing
            await _db.init_db()
        except AttributeError:
            # Fallback to other possible names if the database module exposes them
            if hasattr(_db, "initialize_db"):
                await _db.initialize_db()
            elif hasattr(_db, "db_init"):
                await _db.db_init()
            else:
                # If none exist, raise a clear error (this will be caught by outer error handling)
                raise NameError("database module missing expected init function (init_db/initialize_db/db_init)")

        await _ensure_schema_once(logger)

        # Load blacklist using module alias (defensive — avoids NameError if local import failed)
        if hasattr(_db, "load_blacklist"):
            blacklist = await _db.load_blacklist()
        elif hasattr(_db, "db_load_blacklist"):
            blacklist = await _db.db_load_blacklist()
        else:
            raise NameError("database module missing expected load_blacklist/db_load_blacklist")
        ...
        async with aiohttp.ClientSession() as session:
            while not stop_event.is_set():
                ...
                try:
                    ...
                    # Hygiene
                    if hasattr(_db, "clear_expired_blacklist"):
                        await _db.clear_expired_blacklist(max_age_hours=24)
                    else:
                        # fallback to name used in older versions if present
                        if hasattr(_db, "clear_expired_blacklist"):
                            await _db.clear_expired_blacklist(max_age_hours=24)
                        else:
                            raise NameError("database module missing expected clear_expired_blacklist")

                    if hasattr(_db, "review_blacklist"):
                        await _db.review_blacklist()
                    ...
                    try:
                        if not candidates:
                            ...
                            # Resolve SOL price (attempt multiple sources, cache result)
                            ...
                            if not sol_price:
                                ...
                                try:
                                    max_buys = int((config.get("trading") or {}).get("max_buys_per_cycle", int(os.getenv("MAX_BUYS_PER_CYCLE", "3"))))
                                except Exception:
                                    max_buys = int(os.getenv("MAX_BUYS_PER_CYCLE", "3"))
                                try:
                                    min_buy_sol = float((config.get("trading") or {}).get("min_buy_sol", float(os.getenv("MIN_BUY_SOL", "0.01"))))
                                except Exception:
                                    min_buy_sol = float(os.getenv("MIN_BUY_SOL", "0.01"))
                                try:
                                    buy_cooldown = float((config.get("trading") or {}).get("buy_cooldown_s", float(os.getenv("BUY_COOLDOWN_S", "2"))))
                                except Exception:
                                    buy_cooldown = float(os.getenv("BUY_COOLDOWN_S", "2"))
                                ...
                                for tok in candidates:
                                    ...
                                    try:
                                        ...
                                        if _is_core_stable_like(tok):
                                            logger.debug("Skipping core-stable-like candidate %s (%s)", symbol, token_addr)
                                            continue

                                        # Skip if blacklisted
                                        try:
                                            if token_addr in blacklist:
                                                logger.debug("Skipping blacklisted token %s (%s)", symbol, token_addr)
                                                continue
                                        except Exception:
                                            pass
                                        ...
                                        if per_token_buy_amount_sol < min_buy_sol:
                                            logger.info("Skipping %s (%s): per-token buy amount %.6f < min_buy_sol %.6f",
                                                        symbol, token_addr, per_token_buy_amount_sol, min_buy_sol)
                                            continue

                                        # Affordability check using local snapshot
                                        reserve_buffer = float((config.get("trading") or {}).get("wallet_reserve_sol", 0.0003))
                                        max_affordable = max(0.0, local_balance - reserve_buffer)
                                        if per_token_buy_amount_sol > max_affordable:
                                            logger.info("Not enough available SOL for %s (%s): need %.6f, available %.6f; skipping",
                                                        symbol, token_addr, per_token_buy_amount_sol, max_affordable)
                                            continue

                                        logger.info("Attempting BUY %s (%s) amount=%.6f SOL (available=%.6f)", symbol, token_addr, per_token_buy_amount_sol, local_balance)

                                        # Execute buy - real_on_chain_buy is defensive and handles dry-run/send-disabled internally
                                        try:
                                        ...
                                        if txid:
                                            buys_done += 1
                                            # conservative local balance update; persistent DB write occurs inside real_on_chain_buy
                                            try:
                                        ...
                                        # Per-token protection so one failing token doesn't break the buy loop
                    ...
                    # ---------------- Sells ----------------
                    try:
                        # Resolve get_open_positions safely (may be missing in some builds)
                        ...
                        if not callable(_get_open_positions_fn):
                            # Fallback to DB module if present
                            _get_open_positions_fn = getattr(_db, "get_open_positions", None)
                        ...
                        async def _safe_update_token_record_for_sell(token: dict, sell_txid: str, sell_time: int, is_trading: bool = False):
                            # Prefer top-level update_token_record if present, fall back to _db.update_token_record
                            fn = globals().get("update_token_record")
                            if not callable(fn):
                                fn = getattr(_db, "update_token_record", None)
                            if not callable(fn):
                                # nothing to do; return False to signal no persistence
                                return False
                            try:
                                await fn(token=token, sell_txid=sell_txid, sell_time=sell_time, is_trading=is_trading)
                                return True
                            except Exception:
                        ...
                        for pos in open_positions:
                            try:
                                ...
                                if _dry_run or _simulate or _send_disabled:
                                    txid = _dry_run_txid("SELL")
                                    try:
                                        # Use safe updater that falls back if update_token_record missing
                                        await _safe_update_token_record_for_sell({"address": addr, "symbol": sym}, sell_txid=txid, sell_time=int(time.time()), is_trading=False)
                                    except Exception:
    ...
    # Outer except/finally tail for main()
