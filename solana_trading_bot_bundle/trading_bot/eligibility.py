...
async def is_token_eligible(
    ...
    if not allowed:
        ...
        try:
            rugcheck_ok = True
            # Check if RugCheck is configured/enabled: if so, run the verifier and compare to configured max.
            rcfg = (config.get("rugcheck") or {}) if isinstance(config, dict) else {}
            core_rc_enabled = bool(rcfg.get("enabled", False))
            if core_rc_enabled:
                try:
                    risk_score, risk_labels_raw, _msg = await verify_token_with_rugcheck(token_address, token, session, config)
                except Exception:
                    # If RugCheck call fails, we conservatively treat rugcheck as failing for override purposes.
                ...
                rugcheck_ok = (float(risk_score) <= float(max_rug_score))
            else:
                # RugCheck not enabled; treat as pass for override purposes (caller still must meet volume/liquidity)
                rugcheck_ok = True
        except Exception:
