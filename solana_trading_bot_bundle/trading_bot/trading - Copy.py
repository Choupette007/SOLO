...
async def _persist_dryrun_buy_into_trade_history(
    ...
    """
    Best-effort persist a DRY_RUN buy into the trade_history table so the GUI shows recent buys.
    Controlled by:
      - config["trading"]["persist_dry_run_trades"] == True
      - OR environment PERSIST_DRYRUN_TRADES in {"1","true","yes","on"}
    Behavior:
      - Prefer calling `record_trade(...)` if available (global or _db).
      - Fallback: use _db.connect_db() and a raw INSERT into trade_history.
    This helper swallows exceptions and logs debug-only messages so it cannot block/trip the trading loop.
    """
    try:
        # opt-in guard: do nothing unless operator enabled it
        cfg_flag = False
        try:
        ...
        # First, try a high-level record_trade if present
        try:
            ...
            if callable(rec):
                try:
                    await rec(
                        ...
                        buy_amount=float(buy_amount_sol or 0.0),
                    ...
                    try:
                        await rec(token_address, symbol, float(buy_price or 0.0), float(buy_amount_sol or 0.0), str(buy_txid or ""), int(buy_time or int(time.time())))
                        try:
        ...
        # Fallback: raw INSERT via _db.connect_db()
        try:
...
async def _enrich_tokens_with_price_change_lazy(
    ...
    def _neutral_fallback(src):
        ...
        for t in (src or []):
            ...
            try:
                from .utils import format_market_cap  # type: ignore
            except Exception:
                try:
...
async def has_min_balance(
    ...
    def _extract_lamports(resp: Any) -> Optional[int]:
        try:
            # Common solana-py typed response: has .value which may be int or object
            if resp is None:
                return None
            # If resp has attribute 'value'
            if hasattr(resp, "value"):
                ...
                if hasattr(v, "value"):
                    ...
                    if isinstance(maybe, int):
                        return maybe
                # some stubs use ui_amount etc - not lamports, so ignore
            # If resp has attribute 'result' (RPC raw)
            if hasattr(resp, "result"):
                ...
                if isinstance(res, dict):
                    ...
                    if isinstance(val, int):
                        return val
                    # sometimes value is nested
                    if isinstance(val, dict):
                        if "lamports" in val and isinstance(val["lamports"], int):
                            return val["lamports"]
            ...
            if isinstance(resp, dict):
                ...
                if "value" in resp and isinstance(resp["value"], int):
                    return resp["value"]
            # last-ditch: resp may be a small wrapper object with .lamports
    ...
    try:
        ...
        # try to extract ui_amount and convert to lamports if possible.
        try:
            # ui_amount is SOL, not lamports
            ui_amount = None
            if hasattr(resp, "value"):
            ...
            if isinstance(resp, dict):
                ...
                ui_amount = ui_amount or resp.get("value", {}).get("uiAmount") if isinstance(resp.get("value"), dict) else ui_amount
    ...
    try:
        ...
        if not assoc_addr_exists:
            ...
            if dry_run:
                ...
                # Attempt to create the ATA once
                try:
        ...
        # Persist DRY_RUN buy into trade_history when operator enabled it (non-blocking best-effort)
        try:
            if _skip_onchain_send(config):
                try:
                    ...
                    logger.debug("Persist DRYRUN buy helper raised (ignored)", exc_info=True)
        except Exception:
            # safety: if either _skip_onchain_send or helper fails, ignore
            pass
            ...
            while not stop_event.is_set():
                ...
                try:
                    ...
                    try:
                        ...
                        # Fetch open positions (protected)
                        try:
