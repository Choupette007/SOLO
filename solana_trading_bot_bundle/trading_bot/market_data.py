...
Notable change (2025-09-18):
- create_token_account() now signs locally and submits RAW TX bytes via
  AsyncClient.send_raw_transaction(..., opts=TxOpts(...)) to avoid the
  'send_transaction(..., wallet, opts=...) got multiple values for "opts"' error
  that can occur across different solana-py versions.
- get_sol_price(session, *_ignore, **_kw) remains backward-compatible with
  legacy call sites that pass (session, logger).
"""
...
async def validate_token_mint(token_mint: str, solana_client: AsyncClient) -> bool:
    try:
        ...
        if resp.value is not None:
            logger.debug("Validated token mint %s", token_mint)
            return True
        logger.error("Token supply not found for %s", token_mint)
        return False
    except Exception as e:
        log_error_with_stacktrace(f"Failed to validate token mint {token_mint}", e)
        return False
...
async def create_token_account(
    ...
    try:
        ...
        # Prefer SPL idempotent instruction
        try:
...
async def get_token_balance(
    ...
    try:
        ...
        balance = float(resp.value.ui_amount) if resp.value else 0.0
