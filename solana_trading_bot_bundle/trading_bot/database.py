...
# Defensive import of small helpers from .utils to avoid circular import errors
...
def _connect(dbp: Optional[str] = None) -> _ConnCtx:
    ...
    If dbp is None, use the cached resolved DB path to avoid repeating
    expensive diagnostics and write-tests. To force a re-evaluation (for
    example if config.yaml changes at runtime) call:
        _cached_resolve_db_path.cache_clear()
...
def connect_db(dbp: Optional[str] = None) -> _ConnCtx:
    ...
    Returns the same async context manager as _connect.
    """
    return _connect(dbp)
