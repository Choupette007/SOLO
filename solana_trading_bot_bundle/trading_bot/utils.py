...
class CircuitBreaker429:
    ...
    def record(self, is_429: bool) -> None:
        if is_429:
            ...
            # reset on any successful non-429 event
            self._count = 0
...
def compute_buy_size_usd(wallet_sol_balance: float, sol_usd_price: float, config: Dict[str, Any]) -> float:
    ...
    # config readers with defaults
    prefer_risk = bool(botcfg.get("prefer_risk_sizing", True))
    max_risk_pct = float(botcfg.get("max_risk_pct_of_portfolio", botcfg.get("max_risk_pct_of_portfolio", 0.05)))
...
async def get_buy_amount(
    ...
    """
    # --- SAFETY: tolerate token=None from callers ---
    token = token or {}
    ...
    per_trade_sol = float(botcfg.get("per_trade_sol", 0.0))  # if >0, can cap in SOL
    buy_percentage = float(botcfg.get("buy_percentage", 0.10))  # (kept for legacy helpers)

    # computed USD target using risk sizing helper
    ...
    # Defensive logging (never let logging cause a crash)
    try:
...
# -----------------------------------------------------------------------------
# (rest of scoring and helpers remain unchanged; embedded earlier in file)
# -----------------------------------------------------------------------------
...
def score_token_conservative(token: Dict[str, Any]) -> float:
    # same as previous conservative implementation...
    if is_flagged(token.get("rugcheck_labels")):
        return 0.0
    status = str(token.get("rugcheck_status") or "").strip().lower()
    if status in {"fail", "blocked"}:
